...
Description: This the UnweightedGraph class that is required to be modified by the assignment.   I've added
a non-recursive method of using depth-first search called dfsNonRecurs() following the directions for the
algorithm provided.
 */
...
public class UnweightedGraph<V> implements Graph<V> {
	...
	*/
	public SearchTree dfsNonRecurs(int v) {
		...
		List<Integer> searchOrder = new ArrayList<>(); // To store DFS order
		Stack<Integer> stack = new Stack<>(); // To keep track of vertices left to visit

		// Step 1: Initialize with the starting vertex
		stack.push(v);
		searchOrder.add(v);
		isVisited[v] = true;

		// Step 2: Process the stack until it's empty
		while (!stack.isEmpty()) {
			int x = stack.peek(); // Peek the top vertex in the stack

			// If there are no neighbors left to explore
			if (neighbors.get(x).isEmpty()) {
				stack.pop();
			} else {
				// Explore neighbors
				for (int i = neighbors.get(x).size() - 1; i >= 0; i--) {
					Edge e = neighbors.get(x).get(i); // Grab the edge
					neighbors.get(x).remove(i); // Remove the edge from neighbors list

					if (!isVisited[e.v]) { // If the ending vertex is not visited
						parent[e.v] = x; // Set the parent of the ending vertex
						stack.push(e.v); // Push the ending vertex onto the stack
						searchOrder.add(e.v); // Add it to the DFS order
						isVisited[e.v] = true; // Mark it as visited
						break; // Stop exploring this vertex's neighbors
					}
				}
			}
		}

		// Step 3: Return the search tree
		return new SearchTree(v, parent, searchOrder);
	}
